                    ...
                    if( deviceIndex==patchIndex ) {
                        for (int j=0; j<[allPresetClass.savedAllPreset count]; j++) {
                            ...
                            if (tag==numberOfAllPreset) {
                                ...
                                [allPresetClass toString:[theArrayTest mutableCopy] thatView:@"controlbar" action:@"*"]; /* send JSON form */
                            }
                        }
                    }
                }
...
- (void)alertView:(FUIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==1) {
        ...
        [theArrayTest addObject:[test mutableCopy]];
        [allPresetClass toString:[theArrayTest mutableCopy] thatView:@"allpreset" action:@"!"]; /* send JSON form */
