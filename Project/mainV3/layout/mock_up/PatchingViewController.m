...
-(void)initVariable {
    ...
    createData = [[NSMutableArray alloc] initWithObjects:@"dimmer",@"pan",@"tilt",@"gobo",@"color",@"iris",@"shutter",@"focus",@"zoom", nil];
    start_channel = 0;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1 && alertView.tag == 5000) {
        ...
        for (int i=0; i<row; i++) {
            ...
            [self toString:[theArrayTest mutableCopy]];     /* send JSON form */
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [patchingClass.device removeObjectAtIndex:indexPath.row-1];
        NSLog(@"delete=%@",[patchingClass.device objectAtIndex:indexPath.row-1]);
        //[self resetDeviceID];
