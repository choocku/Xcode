...
-(id)initWithFrame:(CGRect)frame{
    ...
    if(self){
        ...
        _textField.textColor = [UIColor cloudsColor];
        _textField.textAlignment = NSTextAlignmentCenter;
        _textField.font = font;
        _textField.text = [NSString stringWithFormat:@"%d",255*self.angle/360];
        [_textField addTarget:self action:@selector(drawing:) forControlEvents:UIControlEventEditingChanged];
...
-(void)drawing:(CGPoint)lastPoint {
    NSLog(@"drawing");
    
    //Store the new angle
    self.angle = 360*[_textField.text intValue]/255;
...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextAddArc(ctx, self.frame.size.width/2, self.frame.size.height/2, radius-9.6, 0, M_PI *2, 0);
    CGContextSetFillColorWithColor(ctx2, [UIColor midnightBlueColor].CGColor);
...
//** Draw the circle (using a clipped gradient) **/
    ...
    CGContextSetShadowWithColor(imageCtx, CGSizeMake(0, 0), self.angle/20, [UIColor clearColor].CGColor);
    ...
    CGFloat components[8] = {
        0.204, 0.596, 0.859, 1,     // Start color - Blue
        0.204, 0.596, 0.859, 1 };   // End color - Violet
...
-(void) drawTheHandle:(CGContextRef)ctx{
    ...
    //I Love shadows
    CGContextSetShadowWithColor(ctx, CGSizeMake(0, 0), 3, [UIColor clearColor].CGColor);
