        ...
        if (!deviceClass.deviceOnce) {
            ...
            for (int i=0; i<[[json valueForKey:@"groupdevice"]count]; i++) {
                ...
                for (int i=0; i<[[response valueForKey:@"device"] count]; i++) {
                    int device = [[[[response valueForKey:@"device"] objectAtIndex:i] mutableCopy] intValue];
                    [temp2 addObject:[NSNumber numberWithInt:device]];
                }
...
-(void)initVariables {
    ...
    x_groupView = 10;
    y_groupView = 74;
    width_groupView = self.view.bounds.size.width-20;
...
-(void)setGroupDeviceButton {
    ...
    int width_groupDeviceButton = 125-x_groupDeviceButton;
    int height_groupDeviceButton = 125-y_groupDeviceButton;
    for (int i=0; i<row; i++) {
...
-(void)setSelectedGroup:(long)tag {
    @try {
        UILabel *number_hasselect = (UILabel *)(FUIButton *)[myScroll viewWithTag:1000+tag];
        UILabel *name_hasselect = (UILabel *)(FUIButton *)[myScroll viewWithTag:10000+tag];
        UILabel *group_hasselect = (UILabel *)(FUIButton *)[myScroll viewWithTag:100000+tag];
        ...
        tempGroupSelected = [[[deviceClass.saveGroupDevice objectAtIndex:tag-1] objectAtIndex:1] mutableCopy];
        ...
        if (common.count == tempGroupSelected.count) {
            ...
            //NSLog(@"tag%ld white",tag);
            number_hasselect.textColor = [UIColor whiteColor];
            name_hasselect.textColor = [UIColor whiteColor];
            group_hasselect.textColor = [UIColor whiteColor];
        }
    }
    @catch (NSException *exception) {
        NSLog(@"don't have any group device");
    }
