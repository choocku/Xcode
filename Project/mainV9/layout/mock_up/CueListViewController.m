...
-(void)initPatterns {
    ...
    [stopButton setBackgroundImage:[UIImage imageNamed:@"stopButton.png"] forState:UIControlStateNormal];
    stopButton.tag = 103;
    ...
    [forwardButton setBackgroundImage:[UIImage imageNamed:@"forwardButton.png"] forState:UIControlStateNormal];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1 && alertView.tag == 100) {
        ...
        for (int i=0; i<[cueListClass.selected count]; i++) {
            int selectIndex = [[cueListClass.selected objectAtIndex:i] intValue]-1;
            int testIndex = [[[cueListClass.device objectAtIndex:selectIndex] objectAtIndex:0] intValue]-1;
            if( selectIndex==testIndex ) {
                ...
                [test setObject:[NSString stringWithFormat:@"%d",selectIndex+1] forKey:[NSString stringWithFormat:@"device"]];
                for (int j=5; j<[cueListClass.nameCH count]; j++) {
                    [test setObject:[[cueListClass.device objectAtIndex:selectIndex] objectAtIndex:j]
                             forKey:[NSString stringWithFormat:@"%@",[cueListClass.nameCH objectAtIndex:j]]];
                }
