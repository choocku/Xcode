...
- (void)viewDidLoad {
    ...
    if (layoutClass.selected.count!=0) {
        ...
        for (int i=0; i<layoutClass.selected.count; i++) {
            int selectedButton = [[layoutClass.selected objectAtIndex:i] intValue];
            NSLog(@"selectButtonTAG=%d",selectedButton);
            [self setHasSelectedButton:selectedButton];
...
-(void)initButtonFromPatching {
    ...
    for (int i=0; i<layoutClass.patchingData.count; i++) {
        NSLog(@"initButtonFromPatching=%d",i);
        NSString * get_id = [[layoutClass.patchingData objectAtIndex:i] objectAtIndex:0];
