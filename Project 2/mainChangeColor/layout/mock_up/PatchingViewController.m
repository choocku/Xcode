    ...
    @try {
        if (!patchingClass.patchingOnce) {
            ...
            for (int i=0; i<[[json valueForKey:@"patching"]count]; i++) {
                ...
                [temp2 addObject:[[temp objectAtIndex:0] mutableCopy]];
                if ([[temp objectAtIndex:2] isEqual:[patchingClass.typeOfLight objectAtIndex:0]]) {
                    [temp2 addObject:[NSNumber numberWithInt:x_drawDevices-300]];
                    [temp2 addObject:[NSNumber numberWithInt:y_drawDevices]];
                }
                else if ([[temp objectAtIndex:2] isEqual:[patchingClass.typeOfLight objectAtIndex:1]]) {
                    [temp2 addObject:[NSNumber numberWithInt:x_drawDevices]];
                    [temp2 addObject:[NSNumber numberWithInt:y_drawDevices]];
                }
                else if ([[temp objectAtIndex:2] isEqual:[patchingClass.typeOfLight objectAtIndex:2]]) {
                    [temp2 addObject:[NSNumber numberWithInt:x_drawDevices+300]];
                    [temp2 addObject:[NSNumber numberWithInt:y_drawDevices]];
                }
                [patchingClass.layout addObject:[temp2 mutableCopy]];
...
-(void)initPatterns {
    ...
    [self.view setBackgroundColor:[UIColor colorWithRed:0.376 green:0.329 blue:0.282 alpha:1]]; // bfbfbf
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1 && alertView.tag == 5000) {
        ...
        // set type of device
        if (buttonIndex==0)
            type = [patchingClass.typeOfLight objectAtIndex:0];
        else if (buttonIndex==1)
            type = [patchingClass.typeOfLight objectAtIndex:1];
        else if (buttonIndex==2)
            type = [patchingClass.typeOfLight objectAtIndex:2];
        ...
        for (int i=0; i<row; i++) {
            ...
            // set channel
            for(int k=0; k<patchingClass.typeOfLight.count; k++) {
                if ([patchingClass.typeOfLight objectAtIndex:k] == type) {
                    int amount_channel = [[patchingClass.numberChOfLight objectAtIndex:k] intValue];
            ...
            if ([[element objectAtIndex:2] isEqual:[patchingClass.typeOfLight objectAtIndex:0]]) {
                [temp addObject:[NSNumber numberWithInt:x_drawDevices-300]];
                [temp addObject:[NSNumber numberWithInt:y_drawDevices]];
            }
            else if ([[element objectAtIndex:2] isEqual:[patchingClass.typeOfLight objectAtIndex:1]]) {
                [temp addObject:[NSNumber numberWithInt:x_drawDevices]];
                [temp addObject:[NSNumber numberWithInt:y_drawDevices]];
            }
            else if ([[element objectAtIndex:2] isEqual:[patchingClass.typeOfLight objectAtIndex:2]]) {
                [temp addObject:[NSNumber numberWithInt:x_drawDevices+300]];
                [temp addObject:[NSNumber numberWithInt:y_drawDevices]];
            }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [patchingClass.layout removeObjectAtIndex:indexPath.row-1];
